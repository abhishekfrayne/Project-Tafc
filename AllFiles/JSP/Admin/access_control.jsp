		 ...
		 <p>
				...
				<form name="f1" action="<%=request.getContextPath()%>/ControlFileAcess" method="post">
					...
					<tr align="center">
							...
							<td width="75px">:</td>
							<td width="200px">
								<select name="rdate" class="field1" required="yes">
								    	<option value="0">--Select--</option>
								    	<%
								    	      String username1 = (String) session.getAttribute("username");
								    	      int ownerId1 = adminDAO.getDataOwnerId(username1);
								    	      rs = adminDAO.getUploadedFiles(ownerId1);
								    	      
								    		  while(rs.next())
								    	      {
								    	%>
								    	 
								    	<option value="<%=rs.getDate(4)%>"><%=rs.getDate(4)%></option>
								    	<%
								    	      }
								    	%>
					...
					<tr align="center">
							...
							<td width="75px">:</td>
							<td width="200px">
								<select name="rtime" class="field1" required="yes">
								    	<option value="0">--Select--</option>
								    	<%
								    	      String username2 = (String) session.getAttribute("username");
								    	      int ownerId2 = adminDAO.getDataOwnerId(username2);
								    	      rs = adminDAO.getUploadedFiles(ownerId2);
								    	      
								    		  while(rs.next())
								    	      {
								    	%>
								    	 
								    	<option value="<%=rs.getTime(5)%>"><%=rs.getTime(5)%></option>
								    	<%
								    	      }
								    	%>
