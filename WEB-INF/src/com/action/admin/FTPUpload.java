		   ...
		   if(session != null)
			{
			  ...
			  uploadSubject = session.getAttribute("subject").toString();
			   ...
			   rdate=(String) session.getAttribute("date");
			  System.out.println("rdate in ftpupload.java:"+rdate);
			 rtime=(String) session.getAttribute("time");
			  System.out.println("rtime in ftpupload.java:"+rtime);
	       ...
	       { 
				...
				try
				{
			     ...
			     //   byte des_by[]=ConversionProcess.toByteArray(file);
    				...
    				FileUpload1.upload(ftpserver,ftpusername,ftppassword,filename,file,dirToUploadFile);
				...
				//	flag = adminDAO.addUploadTransaction(filename,fileType,date,uploadSubject,c_id,ownerId);
					flag = adminDAO.addUploadTransaction(filename,fileType,rdate,rtime,uploadSubject,c_id,ownerId);
					...
					if(flag)
					{
						...
						rd=req.getRequestDispatcher("/AllFiles/JSP/Admin/select_file.jsp?no=1&file_name="+filename+"&cloud="+c_id+"&rdate="+rdate+"&rtime="+rtime+"");
						rd.forward(req,res);
					}
					else
					{
