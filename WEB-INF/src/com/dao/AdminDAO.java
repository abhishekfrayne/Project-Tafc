...
/* Getting The Number Of Cloud(Starts)*/
	...
	public static String getCloud()
	{
		...
		try
		{
			...
			connection=dao.connector();
			statement = connection.createStatement();
			sql = "SELECT c_url FROM m_cloud where c_flag='1'";
			System.out.println("****** Total Number Of Clouds *******");
			System.out.println(sql);
			resultSet = statement.executeQuery(sql);
			while(resultSet.next())
			{
				clouds = resultSet.getString(1);
			}
...
/* Add Upload Transaction (Starts) */
	...
	public boolean addUploadTransaction(String ufileName,String ufileType, String udate,String utime,String subject,int cloudId,int ownerId) 
	{
		boolean flag=false;
		String sql="";
		try
		{
			...
			connection=dao.connector();
			statement = connection.createStatement();
			sql="insert into m_file_upload (f_name,f_type,f_upload_date,f_upload_time,f_subject,cloud_id,owner_id) values('"+ufileName+"','"+ufileType+"','"+udate+"','"+utime+"','"+subject+"','"+cloudId+"','"+ownerId+"')";
			System.out.println(sql);
			int i=statement.executeUpdate(sql);
			if(i!=0)
			{
...
/* Get Upload File Details By fileId(Starts) */
	...
	public ResultSet getUploadFiles1(int fid)
	{
		try
		{
			DAO dao=DAO.getInstance();
			connection=dao.connector();
			statement = connection.createStatement();
			resultSet = statement.executeQuery("select * from m_file_upload where f_code='"+fid+"'");
		}
		catch(Exception e)
		{
			System.out.println("Opp's Error is in AdminDAO-getUploadedFiles(int ownerId) : ");
...
/* Add AcessControl Details (Starts) */
	...
	public boolean addAcessControl(int fileCode,int deptCode,int designationCode,String rdate,String rtime) 
	{
		boolean flag=false;
		String sql = "";
		try
		{
			...
			connection=dao.connector();
			statement = connection.createStatement();
			sql = "insert into m_acess_control(f_code,domain_code,subdomain_code,rdate,rtime) values('"+fileCode+"','"+deptCode+"','"+designationCode+"','"+rdate+"','"+rtime+"')";
...
/* Get Profile Details (Starts) */
	...
	public static ResultSet getOwnerProfile(String username)
	{
		...
		try
		{
			...
			connection=dao.connector();
			statement = connection.createStatement();
			sql="select * from m_owner where name='"+username+"'";
